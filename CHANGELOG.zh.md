...
### 新功能
- **Job Group 搜索**: 在 Job 列表页新增了搜索框，支持按 `Job Group` ID 进行模糊搜索。
- **解析状态优雅处理**:
    - 为正在导入的应用实现了 "PARSING" 状态管理。
    - 当访问正在解析的应用时，相关 API（除 `getApp` 外）会返回 503 状态码及进度提示。
    - 前端会自动拦截该状态，将用户重定向回列表页并显示“正在处理中...”的顶部通知。
- **健壮的 EventLog 解析**:
    - 修复了 `JacksonEventParser` 的文件处理逻辑，增强了对文件损坏的容错性及更新事务的完整性。
    - 优化了解析器逻辑，防止对滚动日志的重复处理。

### 改进
- **指标可视化**: 更新了 Stage 概览指标表，为非 Duration 的时间类指标（如 GC Time）增加了耗时占比显示，提供更直观的性能上下文。

## 0.16.0 - 2026-02-04

### 新功能
- **Job 列表状态染色**: Job 列表中的 Stage ID 现在根据执行结果（成功、失败、运行中或跳过）显示不同颜色，提供即时的视觉反馈。
- **动态列可见性**:
    - 在 Job 列表页和 Job 详情页新增了“指标选择器”卡片。
    - 重构了 Job 和 Stage 表格，支持动态开关性能指标列。
- **增强导航**: 在全局 Stage 列表中增加了可排序的“Job Id”列，支持直接点击跳转到所属 Job 的详情页。

### 修复
- **UI 稳定性**: 修复了由于响应式状态管理导致的前端组件中多个 `TypeError` 和 `ReferenceError` 问题。

## 0.15.0 - 2026-02-04

### 新功能
- **Job 事件时间轴**: 在 Job 详情页新增双图表时间轴，并行展示 Executor 生命周期（添加/移除）与 Stage 存活周期（提交/完成）。
- **滚动日志支持**: 增强了解析器与监控服务，支持多分片/滚动 Spark 事件日志。日志现在按应用 ID 分组并按序号顺序处理。
- **Job 列表增强**:
    - **ID/组拆分**: 将 Job ID 与 Job Group 拆分为独立的、可排序的列。
    - **Stage 追踪**: 新增“Stages Count”（可排序）与“Stage IDs”（列表）列，直观展示每个 Job 所属的 Stage 分布。

### 改进
- **UI/UX**: 精简了 Job DAG 中的 Stage 标签，仅保留 Stage ID 以保持界面整洁。
- **导航修复**: 修复了 Stage 详情页中“所属 Job”链接跳转到列表页而非特定 Job 详情页的 Bug。

### 修复
- **SQL 准确性**: 解决了 Job 级 Executor 指标查询中的列名歧义问题。
