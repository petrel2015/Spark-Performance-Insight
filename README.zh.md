...
在 AI 时代，**本项目** 致力于重塑 Spark 性能分析。以 Spark 为中心，基于 LLM 深度洞察数据，并通过 **MCP (Model Context Protocol)** 开放核心能力，实现自主化与自然语言驱动的性能调优。作为一款深度性能分析系统，它专为解决原生 Spark Web UI/History Server 核心痛点而生，通过 **奖章架构**、**智能诊断** 以及 **多维深度对标** 技术，彻底告别“重放慢”、“信息过载”与“无对比”的性能诊断困境。

> **💡 核心目标：将 EventLog 转化为“可信赖的行动建议”，直接给出答案，而非堆砌数据。**
...
本项目正从“性能监控面板”进化为“可信赖的 AI 性能专家”。

- **阶段 1：结构化证据链** - P95 分位数、倾斜因子及上下文感知。
- **阶段 2：专家规则库** - 10+ 类经典瓶颈识别与安全动作集。
- **阶段 3：AI 诊断生成** - 基于证据的多智能体 (Proposer-Challenger) 诊断。
- **阶段 4：评价反馈闭环** - 工程师反馈与黄金评测集。
- **阶段 5：自适应调优** - A/B 实验与自适应参数调节。

> **查看详细规划：[项目演进路线图](./docs/zh/Roadmap.md)**
