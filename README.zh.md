...
- **扩展性挑战：** 缺乏结构化存储，所有指标必须缓存于内存，处理海量 Task 时极易引发 OOM。
- **查询效率低：** 线性存储不支持索引。在数万个 Stage 或百万级 Task 中进行翻页、排序时体验极差。

### 2. 缺乏量化的横向对比
- **差异难以量化：** 当作业变慢时，很难直接对比不同运行实例或 Stage 间的指标，难以精准定位性能退化的根源。
- **环境变动盲区：** 难以快速识别性能波动是由配置微调、资源分配差异还是硬件环境变化引起的。

## 核心功能

### 1. 经典视图复刻与增强
- **亲切的交互体验：** 深度复刻原生 Spark UI 的列表展示（Jobs, Stages, Tasks）、进度条及描述，降低迁移门槛，让开发人员上手即用。
- **增强的统计面板：** 完整复刻并增强了 Stage 详情，提供 Duration, GC Time, Spill, Shuffle 等指标的五分位数（Min, 25%, Median, 75%, 95%, Max）统计分布。
- **高性能 Task 视图：** 支持百万级 Task 的后端分页与多列组合排序（Shift+Click）。

### 2. 奖章架构数据管道 (Medallion Pipeline)
- **Bronze (原始入库)：** 基于 Jackson 的超高速流式解析。
- **Silver (结构化转换)：** 数据规范化处理，自动恢复逻辑关联，精准识别长尾 Task。
- **Gold (指标聚合)：** 预计算分析宽表，实现秒级 UI 响应。

### 3. 深度存储分析 (Storage Overhaul)
- **持久化追踪：** 全方位展示缓存 RDD 和 DataFrame 的分布情况。
- **结构化 UI：** 通过状态指示灯直观展示存储级别（Memory/Disk/Deserialized）。
- **深层链接：** 每个 RDD 拥有独立 URL，支持直接分享。

### 4. 智能化诊断引擎 (Smart Diagnosis)
- **AI 深度分析：** 集成 **智谱 AI (GLM-4.7)** 与 **OpenAI**，一键生成专家级诊断报告。
- **规则引擎：** 自动识别数据倾斜、GC 压力、磁盘溢写和调度延迟。

### 5. 多维度深度对比 (Benchmarking)
- **跨应用对比：** 支持两个不同 Application 实例间的全指标对标。
- **Stage 专项对标：** 深入对比两个 Stage 的指标分布及 Task 执行轨迹。

### 6. 企业级稳定性
- **OOM 自动恢复：** 引入 DuckDB `CHECKPOINT` 机制与自动重试逻辑。
- **精准时间预估：** 统一使用毫秒级时间戳，提供准确的导入剩余时间预估。
...
1.  **构建项目：**
    该命令会自动构建前端，并利用 Docker Compose 启动应用。
    ```bash
...
### 构建与运行 (Docker Compose 方式)

该方式会同时启动 **Spark Performance Insight** 和 **原生 Spark History Server**，且两者挂载同一个日志目录，极大方便了对比分析。
