...
- [x] job列表增加诊断分数列
- [x] 诊断页签根据job和stage的诊断分数，给出top10的问题jobb和stage。
- [x] storage页签
- [x] 诊断汇总页面（规则）
- [x] 诊断汇总页面（大模型，集成Spring AI，对接）
- [x] (右上角除了app list，增加一个（处理对比）这个意思的英文)。因为我们要对比2个或多个stage/job，这里相当于购物出，点进去后我们再次选择要对比的stage/job进行对比。为对比做准备。
- [] 导航栏页签跳转后，再次点回来还是之前点开的画面（比如筛选列，打不打开卡片，是不是在详情页里）
- [x] Processing file 1/1 （Lines processed xxx）我希望展示的是一个百分比，从处理行数改为处理字节数算百分比。也就是说先去查一下该文件总大小，然后根据目前处理的大小，计算百分比
- [x] Processing file 1/1 （Lines processed xxx）还应当展示一个内容，再解析文件后预计算stage指标或其他指标时，目前页面没有显示。需要将这个环节也更新到提示信息中。
- [x] job group在搜索时，粘贴进去的东西可能有前后空格，增加trim
- [x] job/stage id搜索栏只允许输入十进制数，比如粘贴进去一个e16，现在也能实现，但是不应该允许